<?php
class Nepali_Calender_Date_Data
{
	private $_bs = array(
...
private $nepali_date = array('year' => '', 'month' => '', 'date' => '', 'day' => '', 'nmonth' => '', 'num_day' => '');
private $english_date = array('year' => '', 'month' => '', 'date' => '', 'day' => '', 'emonth' => '', 'num_day' => '');
public $debug_info = "";
		/**
		 * Return day
		 *
		 ...
		 * @return string
		 */
		private function get_day_of_weeks($day)
		{
			switch ($day)
			{
				case 1:
		 ...
		 * @return string
		 */
		private function get_english_months($m)
		{
			$eMonth = FALSE;
			switch ($m)
			{
		 ...
		 * @return bool
		 */
		private function is_in_nepali_range($yy, $mm, $dd)
		{
			if ($yy < 2000 || $yy > 2089)
			{
		...
		public function eng_to_nep($yy, $mm, $dd)
		{
			...
			else
			{
				...
				$numDay = $day;
				$this->nepali_date['year']    = $y;
				$this->nepali_date['month']   = $m;
				$this->nepali_date['date']    = $total_nDays;
				$this->nepali_date['day']     = $this->get_day_of_weeks($day);
				$this->nepali_date['nmonth']  = $this->_get_nepali_month($m);
				$this->nepali_date['num_day'] = $numDay;
		...
		public function nep_to_eng($yy, $mm, $dd)
		{
			...
			// Check for date range
			$chk = $this->is_in_nepali_range($yy, $mm, $dd);
			if ( $chk !== TRUE)
			{
				die($chk);
			}
			else
			{
				...
				$numDay = $day;
				$this->english_date['year']    = $y;
				$this->english_date['month']   = $m;
				$this->english_date['date']    = $total_eDays;
				$this->english_date['day']     = $this->get_day_of_weeks($day);
				$this->english_date['nmonth']  = $this->get_english_months($m);
				$this->english_date['num_day'] = $numDay;
				return $this->english_date;
			}
		}
		
		private function nepali_to_unicode($numArr) {
			$number = '';
		...
		public function convert_to_unicode($year, $month, $date, $day){
			$finalDate = '';
			
			$unicode_month =  '';
			$unicode_day = '';

			$unicode_year = $this->nepali_to_unicode(array_map('intval', str_split($year)));
			$unicode_date = $this->nepali_to_unicode(array_map('intval', str_split($date)));
			...
			switch($month){
				...
				break;
				case 'kartik':
				$unicode_month .= 'कार्तिक';
				break;
				case 'Mangshir':
				$unicode_month .= 'मङ्सिर';
				break;
				case 'Poush':
				$unicode_month .= 'पुष';
				break;
            ...
            switch($day){
            	...
            	break;
            	case 'Friday':
            	$unicode_day .= 'शुक्रवार';
            	break;
            	case 'Saturday':
            	$unicode_day .= 'शनिवार';
            	break;
            }
            $finalDate =  array($unicode_date, $unicode_month, $unicode_year, $unicode_day);
            return $finalDate;
        }
